...
function Project({ title, image, description, techstack, previewLink, githubLink }) {
  return (
    <article className="rounded-xl mt-10 overflow-hidden shadow-xl shadow-slate-300 dark:shadow-slate-900">
      <img style={{width: "100%", height:"200px", objectFit: "cover"}} src={image} alt="" loading="lazy" />
      <div style={{height: "100%"}} className="dark:bg-dark-card p-4">
        <h1  className="dark:text-light-heading font-semibold text-lg pt-1">{title}</h1>
        <p style={{height: "120px"}} className="text-content pt-4 font-light">{description}</p>
        <h3 style={{height: "120px"}} className="text-dark-heading dark:text-light-heading font-medium pt-4">
